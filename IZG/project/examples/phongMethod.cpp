...
void Method::onDraw(Frame&frame,SceneParam const&sceneParam){
  ...
  ctx.prg.uniforms.uniform[0].m4 = sceneParam.view  ;
  ctx.prg.uniforms.uniform[1].m4 = sceneParam.proj  ;
  ctx.prg.uniforms.uniform[2].v3 = sceneParam.light ;
  ctx.prg.uniforms.uniform[3].v3 = sceneParam.camera;
  draw(ctx,sizeof(bunnyIndices)/sizeof(VertexIndex));
}
