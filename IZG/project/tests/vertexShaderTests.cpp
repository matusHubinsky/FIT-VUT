...
SCENARIO("00"){
...

  {
    ...
    if(inVertices.size() != expected.size()){
      ...
      Když se zavolá funkce draw (drawImpl), měl by se zavolat vertex shader.
      když se zavolá funkce takto: draw(ctx,)."<< N <<R".(), znamená to, že by se měl vertex shader pustit )."<< N << R".(x.
      ...
      Funkce draw přibližně odpovídá funkcím glDrawArrays(GL_TRIANGLES,...) a glDrawElements(GL_TRIANGLES,...) z OpenGL.
...

  {
    ...
    if(inVertices.size() != expected.size()){
      std::cerr << R".(
      vertex shader by se měl zavolat )." << N << R".(x, když se zavolá funkce draw(ctx,)."<< N << R".().
...
SCENARIO("02"){
  ...
  draw(ctx,static_cast<uint32_t>(indices.size()));
...
SCENARIO("05"){
  ...
  for(uint32_t i=0;i<nofCases;++i)
  {
    ...
    if(!cmp(unif.uniform[location],value)){
      ...
      Uživatel funkce draw často chce pro své vykreslování nastavit nějaké konstanty (uniformy), třeba matice.
...
SCENARIO("06"){
  ...
  draw(ctx,static_cast<uint32_t>(vert.size()));
...
SCENARIO("07"){
  ...
  draw(ctx,static_cast<uint32_t>(vertToDraw));
