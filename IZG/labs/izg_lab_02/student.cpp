...
void drawLine(int x1, int y1, int x2, int y2, S_RGBA color)
{
    ...
    // prvy pripad : neexistujuca usecka, musim sa vratit z funckie
    if (x1 == x2 && y1 == y2)
    {
    ...
    bool swap = false;
    // druhy pripad : suradnice sa musia vymenit
    // kedze som vymenil suradnice, musim znovu vypocitat aj dx a dy
    if (ABS(dx) < ABS(dy))
    {
        SWAP(x1, y1);
        SWAP(x2, y2);
    ...
    // treti pripad :vymena bodov [x1, y1] a [x2, y2]
    if (x1 > x2)
    {
        SWAP(x1, x2);
        SWAP(y1, y2);
    ...
    for (int x = x1; x <= x2; ++x)
    {
        ...
        if (swap)
        {
            // nastal 2. pripad, zapis do framebufferu musi byt inverzny
            setPixel(y >> FRAC_BITS, x, color);
        }
        else
        {
            // 2. pripad nenastal, vykreslujem normalne
