 ...
 *
 * V případě, že funkce dostane jako parametr queue ==, volejte funkci
 * Queue_Error(QERR_INIT).
 *
 * @param stack Ukazatel na strukturu fronty
 */
void Queue_Init( Queue *queue ) {   
    // kontrola či ukazovateľ na radu nieje NULL
    if (queue == NULL) {
        Queue_Error(QERR_INIT);
    }
    // celú radu vyplním znakom  '*' ktorý označuje prázdne miesto
...
int nextIndex( int index ) {
    // ak by bol index rovný veľkosti rady, zmení na vďaka modulu na 0 a bude ukazocať na začiatok,
    // to nám zabezpečí "kruhovosť" rady 
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
    // kontrola či rada nieje prázdna
...
void Queue_Remove( Queue *queue ) {
    ...
    // posunie zaciatok, čim sa vymaže prvý znak
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
    // kontola či rada nieje prázdna
    if (Queue_IsEmpty(queue)) {
        Queue_Error(QERR_DEQUEUE);
        return;
    }
    // získanie znaku zo začiatku rady
    Queue_Front(queue, dataPtr);
    // odstánenie znaku zo začiatku rady
	Queue_Remove(queue);
}
...
void Queue_Enqueue( Queue *queue, char data ) {
    ...
    // pridanie dát na index označujúceho voľné miesto v rade
    queue -> array[queue -> freeIndex] = data;
    // posunutie indexu označujúceho voľné miesto
