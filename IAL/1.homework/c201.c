...
void List_Dispose( List *list ) {
    ...
    while (currentElementptr != NULL) {
        ...
        ListElementPtr tmpElementptr = currentElementptr -> nextElement;
        free(currentElementptr);
        currentElementptr = tmpElementptr;
    }
    // uvediem zoznam do stavu ako po inicializácií
    list -> firstElement = NULL;
    list -> activeElement = NULL;
}
...
void List_InsertFirst( List *list, int data ) {
    // vytvorím nový prvok a následne skontrolujem či operácia malloc prebehla úspešnes
    ListElementPtr newElementptr = malloc(sizeof(struct ListElement));
    if (newElementptr == NULL) {
        List_Error();
    } 
    newElementptr -> data = data;
    // nasledujúcim prvkom novéhu prvku bude prvý prvok zoznamu
    newElementptr -> nextElement = list -> firstElement;
    // prvý prvok zoznamu nastavím na môj nový prvok
...
void List_DeleteFirst( List *list ) {
    ...
    // vytvorím si dočasnú premmennú s následníkom prvého prvku
    ListElementPtr tmpElementptr = list -> firstElement -> nextElement;
    if (list -> activeElement == list -> firstElement) {
        // ak bol prvý prvok zároveň aj aktívnym prvkom zoznamu, aktita sa stráca
        list -> activeElement = NULL;
    }
    free(list -> firstElement);
    // novým prvým pvkom bude nasledujúci prvok po prvom prvku
    list -> firstElement = tmpElementptr;
}
...
void List_DeleteAfter( List *list ) {
    ...
    if (list -> activeElement -> nextElement != NULL) {
        // vytvorím si dočasnú premmennú s následníkom následníka aktívneho prvku
        ListElementPtr tmpElementptr = list -> activeElement -> nextElement -> nextElement;
        free(list -> activeElement -> nextElement);
        // následníkom aktívneho prvku bude náš dočasný prvok, čiže prvok ktorý bol
        // pôvodne následník nasledovníka aktívneho prvku
        list -> activeElement -> nextElement = tmpElementptr;
    }
}
...
void List_InsertAfter( List *list, int data ) {
    ...
    if (newElementptr != NULL) {
        newElementptr -> data = data;
        // následníkom nového prvku bude následník aktívneho prvku
        newElementptr -> nextElement = list -> activeElement -> nextElement;
        // nový prvok umiestnený za aktívny prvok
